...
export default class Config extends BaseConfig {
  ...
  protected parseConfig(env: any): ConfigData {
    return {
      ...
      server: {
        port: this.convertToNumber(`${this.ENV_PREFIX}SERVICE_HTTP_PORT`, env[`${this.ENV_PREFIX}SERVICE_HTTP_PORT`])
      },
      dapr: {
        host: this.convertToString(`${this.ENV_PREFIX}DAPR_HTTP_HOST`, env[`${this.ENV_PREFIX}DAPR_HTTP_HOST`]),
        port: this.convertToNumber(`${this.ENV_PREFIX}DAPR_HTTP_PORT`, env[`${this.ENV_PREFIX}DAPR_HTTP_PORT`])
      }
    };
  }
}
